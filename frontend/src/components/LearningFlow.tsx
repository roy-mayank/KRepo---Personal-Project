...
import type { LearningPath } from '@/lib/queries'
...
export default function LearningFlow({ path }: LearningFlowProps) {
  ...
  const incrementXp = useCallback((id: string) => {
    setXpMap((prev) => ({ ...prev, [id]: (prev[id] || 0) + 1 }))
  }, [])

  // convert to reactflow nodes and edges
  const nodes: Node[] = path.nodes.map((n, idx) => ({
    id: n.id,
    data: {
      label: (
