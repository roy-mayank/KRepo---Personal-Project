...
function CreateOrgStep({ onComplete }) {
...
function InviteTeamStep({ tenantSlug, onComplete }) {
  ...
  const handleInvite = async () => {
    ...
    for (const { email, role } of valid) {
      try {
        ...
        inviteResults.push({ email: email.trim(), ok: false, message: err.message || 'Failed' })
      }
    }
