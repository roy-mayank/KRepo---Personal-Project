...
function NavItem({ id, label, icon, small, active, onSelect }: NavItemProps) {
...
export default function MainApp() {
  const [active, setActive] = useState<PageId>('chat')
  ...
  return (
    <div className="flex h-screen w-screen bg-black">
      ...
      <main className="flex-1 overflow-hidden dark">
        ...
        {active === 'login' && <LoginPage />}
        {active === 'signup' && <SignupPage />}
        {active === 'oauth' && <LoginPage />}
      </main>
    </div>
  )
}
